...
//        重置相关信息
        ...
        Const.CONTAINER.ENEMY_BULLETS = [];
        Const.CONTAINER.PLAYER_BULLETS = [];
        Const.CONTAINER.EXPLOSIONS = [];
        ...
        this.initBatchNode();//加载精灵表

        this.initAboutInfo();//分数和主机生命

        this.initShip();//主机加载

        this.scheduleUpdate();//时时处理--会调用本类的update方法

        this.schedule(this.scoreCounter, 1);//创建敌机。。每隔一秒
...
//    飞船，子弹等等BatchNode子节点的一个更新
    ...
    updateUI:function () {
        this._tmpScore += 1;
        this._lbLife.setString(Const.LIFE + '');
...
//    分数在这里面加
    ...
    checkIsCollide:function () {
        ...
        for (i = 0; i < Const.CONTAINER.ENEMIES.length; i++) {
            selChild = Const.CONTAINER.ENEMIES[i];
            if (!selChild.active) {
                continue;
            }

            for (var j = 0; j < Const.CONTAINER.PLAYER_BULLETS.length; j++) {
                bulletChild = Const.CONTAINER.PLAYER_BULLETS[j];
        ...
        for (i = 0; i < Const.CONTAINER.ENEMY_BULLETS.length; i++) {
            selChild = Const.CONTAINER.ENEMY_BULLETS[i];
...
//    坚持飞船重生
    checkIsReborn:function () {
        ...
        if (Const.LIFE > 0 && !locShip.active) {
            locShip.born();
        } else if (Const.LIFE <= 0 && !locShip.active) {

            this._ship = null;

            this._state = STATE_GAMEOVER;

            Const.GAME_STATE = Const.GAME_STATE_ENUM.OVER;
