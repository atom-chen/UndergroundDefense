...
var EnemySprite = cc.Sprite.extend({
    ...
    active : true,
    speed : 200,
    bulletSpeed : Const.BULLET_SPEED.ENEMY,
    HP : 15,
    bulletPowerValue : 1,
    moveType : null,
    scoreValue : 200,
    ...
    shoot:function () {
        ...
        var b = BulletSprite.getOrCreateBullet(this.bulletSpeed, "W2.png", this.attackMode, 3000, Const.UNIT_TAG.ENMEY_BULLET);
        b.x = x;
...
EnemySprite.getOrCreateEnemy = function (arg) {
    var selChild = null;
    for (var j = 0; j < Const.CONTAINER.ENEMIES.length; j++) {
        selChild = Const.CONTAINER.ENEMIES[j];
...
EnemySprite.create = function (arg) {
    ...
    g_GPTouchLayer.addEnemy(enemy, enemy.zOrder, Const.UNIT_TAG.ENEMY);
    Const.CONTAINER.ENEMIES.push(enemy);
    return enemy;
};
