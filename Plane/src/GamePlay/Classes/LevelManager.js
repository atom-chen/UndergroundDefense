...
var LevelManager = cc.Class.extend({
    ...
    loadLevelResource:function(deltaTime){
        ...
        if(Const.ACTIVE_ENEMIES >= this._currentLevel.enemyMax){
            return;
        }
        //load enemy
...
//                            cc.log("jinlai l .2");
    ...
    addEnemyToGameLayer:function(enemyType){
        ...
        addEnemy.x = 80 + (Const.w - 160) * Math.random();
        addEnemy.y = Const.h;
        ...
        switch (addEnemy.moveType) {
            case Const.ENEMY_MOVE_TYPE.ATTACK:
                offset = cc.p(this._gamePlayLayer._ship.x, this._gamePlayLayer._ship.y);
                tmpAction = cc.MoveTo.create(1, offset);
                break;
            case Const.ENEMY_MOVE_TYPE.VERTICAL:
                offset = cc.p(0, -Const.h - addEnemy.height);
                tmpAction = cc.MoveBy.create(4, offset);
                break;
            case Const.ENEMY_MOVE_TYPE.HORIZONTAL:
                ...
                break;
            case Const.ENEMY_MOVE_TYPE.OVERLAP:
                var newX = (addEnemy.x <= Const.w / 2) ? 320 : -320;
